...
[Desktop Entry]
Type=Application
Exec=$SCRIPT_DIR/autostart-terminal.sh bash -c '$(realpath "$0") $TEST_SCRIPT; exec bash'
Hidden=false
NoDisplay=false
...
if declare -f _session_is_cosmic >/dev/null 2>&1 && _session_is_cosmic; then
    ...
    _ut_cosmic=0
    [ "${XDG_SESSION_DESKTOP:-}" = COSMIC ] && _ut_cosmic=1
    case ":${XDG_CURRENT_DESKTOP:-}:" in *:COSMIC:*) _ut_cosmic=1 ;; esac
    if [ "$_ut_cosmic" = 1 ] && command -v cosmic-idle >/dev/null 2>&1; then
    ...
    elif [ "$_ut_cosmic" = 0 ] && command -v gsettings >/dev/null 2>&1; then
        current_lock=$(gsettings get org.gnome.desktop.screensaver lock-enabled 2>/dev/null || echo "error")
